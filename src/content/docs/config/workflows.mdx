...
The `RequestedToActivateExpiry` causes requests to expire if they are approved but not *activated* for a specified amount of time following being requested.
...
This means that even if a user matches more than one of the approval criteria they will only satisfy one of the conditions.The other step will require action from a different user.
