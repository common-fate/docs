...
+ RequestedToApprovedExpiry = 60 * 60 * 4
+ RequestedToActivateExpiry = 60 * 60 * 4
}
```

The `activation_expiry` cause requests to expire if they are approved but not *activated* for a specified amount of time following being approved.

The `RequestedToApprovedExpiry` cause requests to expire if they are requested but not *approved* for a specified amount of time following being requested.

The `RequestedToActivateExpiry` cause requests to expire if they are approved but not *activated* for a specified amount of time following being requested.
...
Multi-step approvals are designed to require each step to map to a different review condition.
This means that even if 1 user matches more than one of the approval criteria they will only satisfy one of the conditions.The other step will require action from a different user.
