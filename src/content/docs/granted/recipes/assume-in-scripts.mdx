...
def assume_role(account):
    ...
    for line in result.stdout.splitlines():
        if line.startswith("AWS_"):  # Capture only AWS-related environment variables
            key, value = line.split("=", 1)
            os.environ[key] = value.strip()
            print(f"✅ Set {key} in Python environment.")  # Log each captured variable
...
if __name__ == "__main__":
    account = "example.org/AWSAdministratorAccess"
    try:
        assume_role(account)
