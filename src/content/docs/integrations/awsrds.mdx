...
Sometimes it may be necessary to connect using a different endpoint than the default configured in the database module above. To solve this the database users configuration have an optional `endpoint` field which can be used to override the endpoint specified in the module.
